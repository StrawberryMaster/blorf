...
function calculateLayerWind(lon, lat, systems) {
    if (!systems || systems.length === 0) return { u: 0, v: 0 };

    const dDeg = 0.5;
    const f = 0.0001458423 * Math.sin(lat * DEG_TO_RAD);
    ...
    const factor = 0.0006 / effectiveF;

    return {
        u: -(p_y_plus - p_y_minus) * factor,
        v: (p_x_plus - p_x_minus) * factor
    };
}

const PI_HALF = Math.PI / 2;
const EXP_MINUS_TWO = 0.135335;
const FADE_SCALE = 1.156517;

export function getWindVectorAt(lon, lat, month, cyclone, pressureSystems) {
    let k = 1.0;
    let inflowAngleRad = 15 * DEG_TO_RAD;
    const landInfo = getLandStatus(lon, lat);

    if (landInfo && landInfo.isLand) {
        const elevation = getElevationAt(lon, lat) || 0;
        k = Math.max(0.4, 0.8 - (elevation * 0.000588)); // elevation / 1700
        inflowAngleRad = Math.min(55, 15 + (elevation * 0.0588)) * DEG_TO_RAD; // elevation / 17
    }

    const envWind = calculateLayerWind(lon, lat, pressureSystems.lower);

    if (!cyclone || cyclone.status !== 'active') {
        return { u: envWind.u, v: envWind.v, magnitude: Math.hypot(envWind.u, envWind.v) };
    }

    const avgLatRad = (lat + cyclone.lat) * 0.5 * DEG_TO_RAD;
    let dxDist = lon - cyclone.lon;
    if (dxDist > 180) dxDist -= 360; else if (dxDist < -180) dxDist += 360;
    dxDist *= Math.cos(avgLatRad);

    const dyDist = lat - cyclone.lat;
    const dist = Math.sqrt(dxDist * dxDist + dyDist * dyDist) * KM_PER_DEG;

    const RMW = 5 + cyclone.circulationSize * 0.125;
    const outerRadius = cyclone.circulationSize * 4.0;

    // if outside the vortex influence, return background wind immediately
    if (dist >= outerRadius) {
        return { u: envWind.u, v: envWind.v, magnitude: Math.hypot(envWind.u, envWind.v) };
    }
    ...
    let vortexSpeed = 0;
    if (dist < RMW) {
        vortexSpeed = cyclone.intensity * (dist / RMW);
    } else {
        const decayExponent = 0.80 - cyclone.circulationSize * 0.0002;
        vortexSpeed = cyclone.intensity * Math.pow(RMW / dist, decayExponent);

        const fadeStart = outerRadius * 0.35;
        if (dist > fadeStart) {
            const t = (dist - fadeStart) / (outerRadius - fadeStart);
            vortexSpeed *= (Math.exp(-2 * t) - EXP_MINUS_TWO) * FADE_SCALE;
        }
    }

    const angleToCenter = Math.atan2(dyDist, dxDist);
    const rotationOffset = (cyclone.lat >= 0) ? (PI_HALF + inflowAngleRad) : (-PI_HALF - inflowAngleRad);
    const windAngle = angleToCenter + rotationOffset;

    const u_vortex = Math.cos(windAngle) * vortexSpeed;
    const v_vortex = Math.sin(windAngle) * vortexSpeed;

    const moveAngleRad = ((450 - cyclone.direction) % 360) * DEG_TO_RAD;
    const transDecay = dist > RMW ? Math.max(0, 1 - (dist - RMW) / (outerRadius - RMW)) : 1.0;

    const transSpeed = cyclone.speed * 0.6 * transDecay; // 0.6 is asymmetryFactor
    const u_trans = Math.cos(moveAngleRad) * transSpeed;
    const v_trans = Math.sin(moveAngleRad) * transSpeed;
...
export function calculateSteering(lon, lat, pressureSystemsObj, bias = { u: 0, v: 0 }) {
    ...
    // deep layer mean (upper 80% / lower 20%)
    const steerU = 0.7 * (windUpper.u * 0.8 + windLower.u * 0.2) + bias.u;
    const steerV = 0.7 * (windUpper.v * 0.8 + windLower.v * 0.2) + bias.v;

    // beta drift
    const latRad = lat * DEG_TO_RAD;
    const betaFactor = Math.sin(latRad < 0 ? 1.2 * latRad - 0.261799 : 1.2 * latRad + 0.261799);
    ...
    return {
        steerU: steerU + betaU,
        steerV: steerV + betaV,
        shearU: windUpper.u - windLower.u,
        shearV: windUpper.v - windLower.v
    };
}
...
export function updateCycloneState(cyclone, pressureSystems, frontalZone, world, month, globalTemp, globalShearSetting, nameIndex) {
    ...
    // ACE calculation (optimized multiplication)
    if (updatedCyclone.age % 6 === 0 && updatedCyclone.intensity >= 34 && !updatedCyclone.isExtratropical) {
        updatedCyclone.ace += (updatedCyclone.intensity * updatedCyclone.intensity) * 0.0001;
    }
    ...
    let totalShear = physicalShear * (globalShearSetting * 0.01);
    const isWinterHalf = (month >= 11 || month <= 4);
    const shearMult = globalShearSetting * globalShearSetting * 0.0001;
    const shearEventProb = (isWinterHalf && updatedCyclone.lon > 100 && updatedCyclone.lon < 121 && updatedCyclone.lat > 16)
        ? 0.55 : (isWinterHalf ? 0.045 * shearMult : 0.03 * shearMult);
    ...
    if (updatedCyclone.shearEventActive) {
        ...
        updatedCyclone.shearEventMagnitude = -3 + Math.random() * 6 + 1.8 * Math.sqrt(Math.abs(month - 8)) + Math.max(0, (globalShearSetting * 0.1 - 10));
    }
    ...
    updatedCyclone.speed += (steeringSpeedKnots - updatedCyclone.speed) * (0.3 + Math.max(0, updatedCyclone.lat * 0.01));

    // upwelling/cold water mixing
    if (updatedCyclone.speed < 6) {
        const coolingRate = (6 - updatedCyclone.speed) * 0.04166; // / 6 * 0.25
    ...
    if (terrainElevation > 0 && updatedCyclone.intensity > 45) {
        // high terrain decay
        let weakeningFactor = 0.88 + updatedCyclone.circulationSize * 0.0001 * EXf - (terrainElevation * 0.000833);
        ...
        let latF = (0.4 / (updatedCyclone.lat * updatedCyclone.lat)) * (updatedCyclone.intensity * 0.02);
        let ri = Math.random() > 0.97 ? Math.random() * 0.35 - 0.05 : 0;
        let intensificationRate = Math.random() * (0.14 + ri) * Math.min(1, ((updatedCyclone.intensity - 13) * 0.01538)) - latF;

        if (updatedCyclone.isMonsoonDepression) intensificationRate *= (Math.random() + 0.10) * 0.70;

        const potentialChange = (mpi - updatedCyclone.intensity) * intensificationRate;

        // shear penalty
        let shear = totalShear * 0.1;
        const nioShearBoost = (updatedCyclone.lat >= 5 && updatedCyclone.lat <= 30 && updatedCyclone.lon >= 30 && updatedCyclone.lon <= 100) ? 8.5 : 0;
        const shemShearBoost = (updatedCyclone.lat <= -5 && updatedCyclone.lat >= -30 && updatedCyclone.lon >= 100) ? (25.0 * Math.sin((month - 2) * 0.523598)) : 0;

        let baseGradient = updatedCyclone.lat > 0 ? (2.0 * Math.cos((month - 2) * 0.523598)) : (1.5 * Math.sin((month - 2) * 0.523598));
        let highLatCorrection = Math.abs(updatedCyclone.lat) > 35 ? Math.pow(Math.abs(updatedCyclone.lat) - 35, 0.9) * -0.1 : 0;

        shear += Math.max(0, (Math.abs(updatedCyclone.lat) * (baseGradient + highLatCorrection) - 30 + nioShearBoost + shemShearBoost)) * 0.05;

        // dry air/humidity penalty
        const samplingRadiusDeg = cyclone.circulationSize * 0.005;
        const cosLat = 1.0 / Math.max(0.1, Math.cos(cyclone.lat * DEG_TO_RAD));
        let envHumiditySum = 0;
        let minEnvHumidity = 60;
        const samplePoints = 8;
        const angleStep = 0.785398; // 2 * PI / 8

        for (let i = 0; i < samplePoints; i++) {
            const angleRad = i * angleStep;
            const sampleLon = cyclone.lon + samplingRadiusDeg * Math.cos(angleRad) * cosLat;
        ...
        const sizeFactor = 1.2 + (clampedSize - 150) * -0.001142; // (0.8 - 1.2) / 350
    ...
    const measureRadius = (rad, threshold) => {
    ...
    let newLon = updatedCyclone.lon + distanceDeg * Math.cos(angleRad) * lonScale;
